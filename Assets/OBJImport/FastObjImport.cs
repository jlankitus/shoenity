 ...
 * Example usage: Mesh myMesh = FastObjImport.Instance.ImportFile("path_to_obj_file.obj");
 */
...
public sealed class FastObjImport
{
 
    #region singleton
    // Singleton code
    // Static can be called from anywhere without having to make an instance
    private static FastObjImport _instance;
 
    // If called check if there is an instance, otherwise create it
    public static FastObjImport Instance
    {
        get { return _instance ?? (_instance = new FastObjImport()); }
    }
    #endregion
